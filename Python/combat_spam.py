...
def is_spam(graph, source, target, k):
    """ Returns the number of nodes from source to target """
    # a -> a k=0 case
    if k == 0 and source == target:
        return target in graph[source]
    # a->a, a->b, b->a k=1 case
    if k == 1:
        if target in graph[source] and source in graph[target]:
            return True
    if k > 1:
        return linked(graph, source, target, k)
    return False

def linked(graph, source, target, k):
    if k == 0:
        return False
    if source in graph[target]:
        return True
    for link in graph[target]:
        if linked(graph, source, link, k-1):
          return True
    return False
...
def compute_ranks(graph, k):
    ...
    for i in range(0, numloops):
        ...
        for page in graph:
            newrank = (1 - d) / npages
            for node in graph:
                if page in graph[node] and not is_spam(graph, node, page, k):
