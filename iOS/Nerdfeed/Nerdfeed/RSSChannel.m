...
- (void)trimItemTitles
{
    ...
    for (RSSItem *i in items)
    {
        ...
        // Find matches in the title string.  The range argument specifies how much of the title to search;
        ...
        if ([matches count] > 0)
        {
            // Print the locatoin of the match in the string and the string itself
...
- (void)readFromJSONDictionary:(NSDictionary *)d
{
    ...
    for (NSDictionary *entry in entries)
    {
        ...
        // Pass the dictionary to the item so it can grab its ivars
...
- (void)addItemsFromChannel:(RSSChannel *)otherChannel
{
    ...
    // Sort the array of items by the publication date
